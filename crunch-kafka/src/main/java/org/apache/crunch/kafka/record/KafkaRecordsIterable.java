...
class KafkaRecordsIterable<K, V> implements Iterable<ConsumerRecord<K, V>> {
   ...
   */
  KafkaRecordsIterable(Consumer<K, V> consumer, Map<TopicPartition, Pair<Long, Long>> offsets, Properties properties) {
    if (consumer == null) {
    ...
    String retryString = properties
        .getProperty(KafkaUtils.KAFKA_RETRY_ATTEMPTS_KEY, KafkaUtils.KAFKA_RETRY_ATTEMPTS_DEFAULT_STRING);
    ...
    scanPollTime = Long.parseLong(
        properties.getProperty(KafkaSource.CONSUMER_POLL_TIMEOUT_KEY, Long.toString(KafkaSource.CONSUMER_POLL_TIMEOUT_DEFAULT)));
  }
  ...
  private static class RecordsIterator<K, V> implements Iterator<ConsumerRecord<K, V>> {
    ...
    RecordsIterator(Consumer<K, V> consumer, Map<TopicPartition, Pair<Long, Long>> offsets, long pollTime, int maxNumRetries) {
      this.consumer = consumer;
